...
The persistence envelope carries a version. A deploy that changes what a phase's stored game state requires must bump the version and add a read-time migration for the immediately-prior version in `src/lib/room/persistence.ts` (see `ROOM_PERSISTENCE_LEGACY_VERSION`), so envelopes already written by the currently-live build keep decoding until they are naturally rewritten (any successful command re-persists at the current version). An envelope tagged with a version this build does not recognize as either current or the one known-prior version fails with `persistence_version_unsupported`, distinct from a merely invalid envelope. There is no forward compatibility: rolling back to a build older than the one that wrote an envelope's version is not supported and will reject that envelope outright.
