...
- `POST /room/custom-amazon-item`: accepts `{ credential, query }`, authorizes the credential as the current trader while the active room is in `generatingItem` with `mode: "Amazon"` and `customAmazonQuery: true`, generates the item from the query, stores the private item, dispatches `ITEM_RECEIVED`, and returns the public snapshot.
